...
class TraineeStorageLoaderTest {
    ...
    @Mock
    private GenericUserNameVerifier<Trainee> traineeUserNameVerifier;
    ...
    void testLoadValidTrainees() {
        ...
        try (MockedStatic<FileValidator> fileValidatorMock = mockStatic(FileValidator.class)) {
            ...
            when(traineeUserNameVerifier.verifyUserNames(anyList(), anyMap())).thenReturn(Collections.emptyList());
    ...
    void testLoadDuplicateTrainees() {
        traineeStorageLoader = new TraineeStorageLoader(resourceLoader, duplicateTraineeFilePath, new FileValidator(), traineeFileReader, traineeValidator, traineeUserNameVerifier);
        try (MockedStatic<FileValidator> fileValidatorMock = mockStatic(FileValidator.class)) {
    ...
    void testLoadInvalidNameTrainees() {
        traineeStorageLoader = new TraineeStorageLoader(resourceLoader, invalidNameTraineeFilePath, new FileValidator(), traineeFileReader, traineeValidator, traineeUserNameVerifier);
        try (MockedStatic<FileValidator> fileValidatorMock = mockStatic(FileValidator.class)) {
    ...
    void testLoadNegativeIdTrainees() {
        traineeStorageLoader = new TraineeStorageLoader(resourceLoader, negativeIdTraineeFilePath, new FileValidator(), traineeFileReader, traineeValidator, traineeUserNameVerifier);
        try (MockedStatic<FileValidator> fileValidatorMock = mockStatic(FileValidator.class)) {
